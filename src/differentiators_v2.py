...
Surfaces where the tracked portfolio leads vs competitors using explainable indices:
- Ownership Index (O): Share of topic coverage from the tracked portfolio
...
def compute_global_notes(df_window: pd.DataFrame) -> dict:
    ...
    if int_weekend > comp_weekend:
        weekend_advantage = f"Portfolio leads with {int_weekend:.1f}% weekend coverage vs competitors' {comp_weekend:.1f}%"
    else:
...
*Brief generated by the Competitive Intelligence System*
"""
